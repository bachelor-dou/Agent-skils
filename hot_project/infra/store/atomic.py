...
# fcntl 是**进程间**互斥,同进程内多线程拿同一个 fd 不会互相阻塞,所以两层都需要:
# threading.Lock 挡住本进程的线程,flock 挡住 CI 里并行的另一个进程。
...
class StoreReadError(RuntimeError):
    ...
    单独一个类型,是为了让调用方区分「文件还没有」(可以从默认值起步)和「文件坏了」
    (绝不能当空数据继续走)。
    """
...
def _load(path: Path, default: Any | None) -> Any:
    ...
    try:
        with open(path, encoding="utf-8") as f:
            return json.load(f)
    # UnicodeDecodeError 既不在 OSError 也不在 JSONDecodeError 之下(它是 ValueError),
    # 漏掉它,一个非 UTF-8 字节就会绕过 StoreReadError、以未预期异常的形式冒出去。
...
def _dump(path: Path, data: Any) -> None:
    ...
    try:
        ...
        # 半截 .tmp 和数据文件同名只多个后缀,留着很容易被当成正常文件。
