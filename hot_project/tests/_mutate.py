...
MUTATIONS: list[tuple[str, Path, str, str, str, str]] = [
    ...
    (
        "锚点报请求的窗口而非真实窗口(爆发探针虚高 67% 的那个 bug)",
        STORE / "snapshots.py",
        "    return None if stars is None else Anchor(day, stars, days_between(utc_today(), day))",
        "    return None if stars is None else Anchor(day, stars, 7)",
        STORE_TESTS,
        "test_anchor_reports_the_real_window",
    ),
    (
        "读取层丢掉未知字段",
        STORE / "universe.py",
    ...
    (
        "未决被当成涨了 0(仓库带着假的零增长进排名,永远出不了榜)",
        GROWTH,
        "    return UNRESOLVED",
        "    return Growth(0, UNDECIDED)",
        GROWTH_TESTS,
        "test_old_repo_missing_from_the_anchor_is_undecided_not_zero",
    ),
    (
        "未决混进批量结果(调用方当零增长用)",
        GROWTH,
        "        return {n: g.value for n, g in self.growths.items() if g.value is not None}",
        "        return {n: g.value or 0 for n, g in self.growths.items()}",
        GROWTH_TESTS,
        "test_batch_reports_where_each_number_came_from",
    ),
    (
    ...
    (
        ...
        GROWTH,
        "        return Growth(current_star - anchor_star, ANCHOR)",
        "        return Growth(max(0, current_star - anchor_star), ANCHOR)",
        GROWTH_TESTS,
    ...
    # ── 榜单流水线 ──────────────────────────────────────────
    (
        "今天没快照照样出榜(拿昨天或空池排,榜单看着正常其实是错的)",
        RANKING,
        '        logger.error("今天的快照还没落盘,榜单无从算起(每日任务挂了?)。")\n        return {}',
        '        logger.error("今天的快照还没落盘。")\n        stars = {"x/y": 9999}',
        RANKING_TESTS,
        "test_no_snapshot_today_means_no_ranking_rather_than_a_wrong_one",
    ),
    (
        "锚点顺延后仍按请求天数算(5 天增量当 7 天用,新项目被误判出局)",
        RANKING,
        "        window_days=anchor.window_days,\n    )",
        "        window_days=window_days,\n    )",
        RANKING_TESTS,
        "test_a_deferred_anchor_widens_the_window_everywhere",
    ),
    (
        "探针不过滤掉星的仓库(负的最近增长混进加成)",
        RANKING,
        "if isinstance(base := anchor.stars.get(name), int) and info[\"star\"] >= base",
        "if isinstance(base := anchor.stars.get(name), int)",
        RANKING_TESTS,
