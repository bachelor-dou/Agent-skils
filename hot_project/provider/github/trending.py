...
这是**唯一不吃 token 的采集来源** —— 网页,不是 API,不占 Search 限额。所以它能和另外两个
收集阶段完全并行,顺手补上搜索的盲区:刚发布、star 总数还够不着门槛、但这几天涨得凶的项目,
Search 按 star 排序永远排不到它们。

解析用正则而不是 HTML 解析器:只取五个字段,页面结构自 2024 年起没变过,不值得为它引一个
依赖。代价是页面改版时会静默地解析出 0 条 —— 所以 `parse` 一并返回文章总数,拿它和结果数
一比就知道是「今天榜单短」还是「解析器坏了」。这两件事看起来一样,处置方式完全相反。
"""
