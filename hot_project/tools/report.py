...
按优先级:DB 里没过期的缓存 → LLM 现生成 → 从元数据兜底拼一段。第三档不是摆设:
LLM 全挂时报告照样要能出,一份「只有 star 数没有介绍」的报告仍然有用,
而一次异常会让跑了两小时的流水线颗粒无收。

## 文件名

    2026-07-30.md            综合榜
    2026-07-30_NEW.md        新项目榜
    2026-07-30_KEY_向量库.md  关键词榜(带方向,否则同日两个方向互相覆盖)

窗口偏离默认值时追加 `_10d`;新项目榜的增长窗口偏离时追加 `_win7d`。
"""
...
def _extract(text: str) -> dict[str, str]:
    """从 LLM 输出里按字段名切出四段。认中英文冒号,两种它都会写。

    模型有时会多写「核心依赖与生态」这类没要求的字段。遇到未知字段名要**停止**往
    上一段追加内容,否则多出来的那几百字会全部粘在「技术架构与特性」尾巴上。
    """
    found: dict[str, str] = {}
    current = ""
...
def _needs_llm(saved: dict) -> bool:
    ...
    没有时间戳的旧数据**不**算过期 —— 否则第一次跑新逻辑时全库描述会一次性判定失效,
    一轮里重刷几百个。调用方给它盖上今天的日期(grandfather),之后按正常周期轮转。
    """
...
def descriptions(ranked: list[tuple[str, dict]], saved_by_name: dict[str, dict],
    ...
    要写回的单独给出来,是为了让调用方决定写不写 —— 这个模块自己不碰库。里面有两类:
    新生成的描述,以及**沿用旧描述但缺时间戳**的(补盖今天,见 `_needs_llm`)。

    素材一次性批量抓完再逐个调 LLM:抓取能并发、LLM 只能串行,混在一个循环里会让每个
    项目白等一次网络往返。旧代码就是混着写的。
    """
