...
│                          Phase 1 — 统一收集                             │
│  ┌────────────────────┐ ┌────────────────────┐ ┌────────────────────┐  │
│  │   关键词搜索        │ │   Star 范围扫描     │ │   GitHub Trending  │  │
...
│  │       No                                                         │  │
│  │       ↓                                                          │  │
│  │  REST 二分法 (线程池) ──422──→ 路径C: GraphQL采样外推 (线程池)    │  │
│  │       │                              │                           │  │
│  │       └──── 返回结果 ←───────────────┘                           │  │
│  │              ↓                                                    │  │
│  │       主线程: update_db + growth >= 800 → candidate_map          │  │
│  └──────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────┬───────────────────────────────────────┘
                                  ↓
┌─────────────────────────────────────────────────────────────────────────┐
...
### 3.2 token_manager.py — Token 轮换管理器

**类 `TokenManager`**：管理多个 GitHub Token 的并发分配、限流感知和自动恢复。

| 方法 | 说明 |
|------|------|
| `acquire_token() → int` | 获取可用 token 索引（全部限流时阻塞等待恢复） |
| `release_token(token_idx, response)` | 归还 token，从响应头更新 `X-RateLimit-Remaining` |
...
| `handle_rate_limit(response, token_idx)` | 处理 403，记录 `X-RateLimit-Reset` 时间戳 |

核心机制：
- **round-robin 轮换**：每次请求自动切到下一个 token
- **限流感知**：`X-RateLimit-Remaining < 10` 时主动标记
- **自动恢复**：全部 token 耗尽时，sleep 到最早 reset 时间
...
| 函数 | 说明 |
|------|------|
| `search_github_repos(token_mgr, query, page, per_page, sort, order, auto_star_filter)` | Search API 搜索（自动附加 `stars:>=1000`，3 次重试） |
| `get_search_total_count(token_mgr, query) → int` | 获取搜索结果总数（per_page=1 只查 count） |
| `auto_split_star_range(token_mgr, low, high, max_results=800, min_span=50)` | **递归分段算法**：查询区间 total_count，超过 800 则二分拆成两个子区间继续递归 |
| `get_stargazers_page(token_mgr, owner, repo, page, per_page)` | REST stargazers 分页查询 |
| `graphql_stargazers_batch(token_mgr, owner, repo, last, before)` | GraphQL 批量获取 stargazers（`last+before` 逆序翻页） |
...
| 函数 | 说明 |
|------|------|
| `estimate_star_growth_binary(token_mgr, owner, repo, total_stars) → int` | REST 二分法（路径 B） |
| `estimate_by_sampling(token_mgr, owner, repo) → int` | GraphQL 采样外推（路径 C） |
...
### 3.11 api_server.py — FastAPI Web 服务
...
│                                                 │                                │
│  ┌──────────────────────────┐                  ↓                                │
│  │  STAR_RANGE 2000~45000   │          ┌───────────────┐                        │
│  │  → auto_split_star_range │──────────→│  raw_repos    │                        │
│  │  → search_github_repos() │          │  {name: info} │                        │
│  └──────────────────────────┘          │  去重 + ≥1000 │                        │
│                                         └───────┬───────┘                        │
│  ┌──────────────────────────┐                  ↑                                │
│  │  GitHub Trending         │                  │                                │
│  │  → fetch_trending()      │──daily/monthly──→┘                                │
│  │                          │──weekly≥800──→ candidate_map (直接入选)            │
│  └──────────────────────────┘                                                   │
└─────────────────────────────────────────────────┬───────────────────────────────┘
                                                  │
                                                  ↓
