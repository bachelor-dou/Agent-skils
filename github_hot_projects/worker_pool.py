...
统一 Worker Pool
=================
3 个 Worker 线程各绑定一个 GitHub Token，从共享任务队列中取任务执行。
...
扩展新任务只需继承 Task 基类，实现 execute(token_idx) 方法。
"""
...
class Task(ABC):
    ...
    4. 可选实现 on_result() / on_error() 自行处理结果
    5. 在 pipeline.py 中 pool.submit() 即可
...
class TokenWorkerPool:
    ...
    def _worker_exit(self, worker_id: int) -> None:
