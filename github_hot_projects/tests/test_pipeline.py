...
class TestDiscoveryPipeline:
    def test_collect_repos_keeps_scan_results_when_seen_is_mutated(self, mock_token_mgr):
        ...
        raw_search_repo = {
            "full_name": "org/search",
            "star": 100,
            "_raw": {"full_name": "org/search", "stargazers_count": 100},
        }
        raw_scan_repo = {
            "full_name": "org/scan",
            "star": 200,
            "_raw": {"full_name": "org/scan", "stargazers_count": 200},
        }

        def fake_scan(_token_mgr, seen_repos=None, **_kwargs):
            if seen_repos is not None:
                seen_repos.add("org/scan")
            return {"repos": [], "total": 1, "_raw_repos": [raw_scan_repo]}

        with patch(
            "github_hot_projects.scheduled_update.tool_search_by_keywords",
            return_value={"repos": [], "total": 1, "_raw_repos": [raw_search_repo]},
        ), patch(
            ...
            repos, seen = pipeline._collect_repos()

        assert [repo["full_name"] for repo in repos] == ["org/search", "org/scan"]
        assert seen == {"org/search", "org/scan"}
    ...
    def test_pipeline_passes_force_refresh_to_growth_stage(self, mock_token_mgr):
        ...
        with patch(
            ...
            result = pipeline.run(force_refresh=True, growth_calc_days=14)

        assert result["report_path"] == "report.md"
        assert mock_batch.call_args.kwargs["growth_calc_days"] == 14
...
def test_scheduled_update_passes_growth_window_and_force_refresh():
    ...
    assert "mode" not in mock_pipeline.return_value.run.call_args.kwargs
    assert mock_pipeline.return_value.run.call_args.kwargs["growth_calc_days"] == 14
