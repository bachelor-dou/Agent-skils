...
class TestRanking:
    def test_comprehensive_basic_sort(self, sample_candidates):
        """综合模式：按 score 降序排列。"""
        from hot_projects.ranking import step2_rank_and_select
    ...
    def test_hot_new_missing_created_at_hydrate(self):
        """hot_new 兜底模式：从 DB 补充 created_at（用相对今天的日期，避免时间敏感）。"""
        from hot_projects.ranking import step2_rank_and_select
        recent = (datetime.now(timezone.utc) - timedelta(days=20)).strftime("%Y-%m-%d")
        db = {"date": "2026-01-01", "valid": True,
              "projects": {"test-org/test-repo": {"star": 5000, "created_at": recent}}}
        candidates = {
            "test-org/test-repo": {"growth": 800, "star": 5000},  # 无 created_at
        }
        result = step2_rank_and_select(
            candidates, mode="hot_new", db=db, days_since_created=60,
        )
        assert len(result) == 1
