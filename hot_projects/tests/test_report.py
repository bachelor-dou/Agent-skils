...
class TestReport:
    def test_generate_report_comprehensive(self, tmp_path):
        ...
        with patch("hot_projects.report.REPORT_DIR", str(tmp_path)):
            with patch("hot_projects.report.call_llm_describe", return_value="测试描述内容"):
                from hot_projects.report import step3_generate_report
    ...
    def test_generate_report_hot_new(self, tmp_path):
        ...
        with patch("hot_projects.report.REPORT_DIR", str(tmp_path)):
            with patch("hot_projects.report.call_llm_describe", return_value="新项目描述"):
                from hot_projects.report import step3_generate_report
    ...
    def test_generate_report_uses_db_cache(self, tmp_path):
        ...
        with patch("hot_projects.report.REPORT_DIR", str(tmp_path)):
            with patch("hot_projects.report.call_llm_describe") as mock_llm:
                from hot_projects.report import step3_generate_report
                path = step3_generate_report(top_projects, db, mode="comprehensive")
    ...
    def test_generate_report_empty_desc_requires_llm(self, tmp_path):
        ...
        detailed_desc = (
            ...
            "已知局限或注意事项：仍需结合源码进一步确认。"
        )

        with patch("hot_projects.report.REPORT_DIR", str(tmp_path)):
            with patch("hot_projects.report.call_llm_describe", return_value=detailed_desc) as mock_llm:
                from hot_projects.report import step3_generate_report
                path = step3_generate_report(top_projects, db, mode="comprehensive")
