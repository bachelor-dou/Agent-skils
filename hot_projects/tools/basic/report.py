...
架构定位：
    tools/basic 组件，step3_generate_report() 由 ranking 编排调用。
...
from ...datasource.github.api import fetch_repo_readme_excerpt, fetch_repo_recent_commits
...
# 专用文件名标识：综合榜=纯日期；新项目榜=_NEW；关键词榜=_KEY；
# 筛选区间非默认时尾部追加 _10d / _win7d 等字样。
_MODE_META = {
...
def step3_generate_report(
    ...
    if mode in ("comprehensive", "keyword") and growth_calc_days != GROWTH_CALC_DAYS:
        suffix = f"{suffix}_{growth_calc_days}d"
        title_prefix = f"{title_prefix}（近{growth_calc_days}天增长）"
    if mode == "hot_new" and days_since_created is not None:
        # 标题始终点明窗口；文件名仅在偏离默认窗口时追加区间尾缀
        if days_since_created != DAYS_SINCE_CREATED:
    ...
    if topic:
        ...
        # 关键词榜文件名带上方向，避免同日多方向互相覆盖（如 _KEY_向量库 / _KEY_OCR）
        if mode == "keyword":
            slug = re.sub(r'[\s/\\:*?"<>|.]+', "", topic)[:6]
            if slug:
                suffix = f"{suffix}_{slug}"
